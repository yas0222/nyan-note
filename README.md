...
18. 猫プロフィール画像のアップロード追加（画像がある場合は絵文字より優先表示）
19. アップロード画像は縮小・圧縮した上で `localStorage` に保存（端末ごとのローカル保存）
...
- 画像未設定時は、これまでどおり「写真（絵文字）」を表示します。
- 保存先は `localStorage` です（クラウド同期なし / 端末ごとのローカル保存）。
