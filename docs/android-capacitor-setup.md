...
### Android ネイティブ Google ログイン（Capacitor）追加設定

Android/Capacitor 版は Web/PWA 版と認証方式を分離して運用する。

- **Web/PWA 版**: 既存の Firebase Web Auth（Popup/Redirect）を維持する。
- **Android/Capacitor 版**: `@capacitor-firebase/authentication` を使ったネイティブ Google 認証を利用する。

必須設定:

1. Firebase Console で **Android アプリを追加**する。
2. Android package id は **`app.nyannote.prototype`** を指定する。
3. デバッグ用 SHA-1 / SHA-256 を Firebase に登録する。
4. 登録後に `google-services.json` をダウンロードし、**`android/app/google-services.json`** に配置する。
5. Firebase Authentication で **Google ログインプロバイダを有効化**する。

補足:

- Android 側でネイティブ Google ログインを実行後、取得した ID トークンを Firebase Web Auth の `GoogleAuthProvider.credential(idToken)` + `signInWithCredential` で連携することで、既存の Firestore 利用ロジックとセッション整合性を保てる。
- Web/PWA 版と Android/Capacitor 版はログイン開始方式が異なるため、不具合調査時はプラットフォーム別に切り分ける。
